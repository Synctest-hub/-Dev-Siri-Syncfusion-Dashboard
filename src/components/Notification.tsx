    ...
    <div className="mt-5 ">
      {chatData?.map((item, index) => (
        <div key={index} className="flex items-center leading-8 gap-5 border-b-1 border-color p-3">
          <img src={item.image} alt={item.message} height={40} width={40} className="rounded-full" />
          <div>
      ...
      <div className="mt-5">
        <Button color="white" useThemeBgColor text="See all notifications" borderRadius="10px" width="full" />
      </div>
    </div>
  </div>
);
